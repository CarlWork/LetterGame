...
class DMFindingGameViewController: UIViewController {
    ...
    func generateRandomLetters(numLetters: Int) -> [String] {
        var letterPool = letters.shuffled()
        
        let result = letterPool[1...(numLetters)]
        var randomLetters = Array(result)
        
        // make sure randomLetters does not contain targetLetter
        guard !randomLetters.contains(targetLetter) else { return randomLetters }
        randomLetters[0] = targetLetter
        // [b,c,a] -> [targetLetter, c, a]
    ...
    @IBAction func tappedLetter(_ sender: UIButton) {
        let selectedLetter = sender.titleLabel
    ...
    func updateLetterButtons() {
        ...
        for index in 0...letterButtons.count - 1 {
